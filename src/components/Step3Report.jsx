...
const Step3Report = ({ report }) => {
  ...
  const downloadPDF = () => {
    ...
    if (finalScore >= 8) {
      ...
      advice = "Good foundation shown. Improve clarity and structure. Practice delivering consise, confident answers with stronger supporting examples."
    }else {
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-gray-50 to-blue-50 px-4 sm:px-6 lg:px-10 py-8'>
      <div className='mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
        <div className='md:mb-10 w-full flex items-start gap-4 '>
          ...
          <div>
            <h1 className='text-3xl font-bold flex-nowrap text-blue-950'>
              Interview Analutics Dashboard
            </h1>
        ...
        <button 
        onClick={downloadPDF}
        className='bg-cyan-600 hover:bg-cyan-700 text-white py-3 rounded-xl shadow-md transition-all duration-300 font-semibold text-sm sm:text-base text-nowrap p-2 cursor-pointer'>
          Download PDF
        </button>
      </div>
              ...
              {questionWiseScore.map((q,i)=>(
                <div key={i} className='bg-gray-50 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200'>
                  ...
                  <div className='bg-blue-50 border border-blue-200 p-4 rounded-lg'>
                    ...
                    <p className='text-sm text-gray-700 leading-relaxed'>{q.feedback && q.feedback.trim() !== ""
                      ? q.feedback : "No feedback available for this question."}</p>
                  </div>
                </div>
              ))}
            </div>
